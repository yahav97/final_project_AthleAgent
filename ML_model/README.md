...
| `train_model.py` | Train candidates → `artifacts/<run_id>/` |
...
|------|-------|
| Winner | `XGBoostDeep` |
| Promoted run | `artifacts/20260512_075115/` |
| Features | 36 (`backend/data/model_feature_contract.json`) |
| Decision threshold (manifest) | `0.18` — Recall/Precision metrics at promotion |
...
> Manifest `policy.recall_min: 0.85` is training policy — not the runtime hard gate.
...
├── injury_model.pkl      # joblib bundle (XGBoost + preprocessing)
