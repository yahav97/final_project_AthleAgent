...
def pick_best_model_for_name(
    results_df: pd.DataFrame,
    threshold_rows: list[dict[str, float | str]],
    model_name: str,
) -> pd.Series:
    """Holdout policy row for one named candidate."""
    subset = results_df[results_df["Model"] == model_name]
    if subset.empty:
        raise ValueError(f"Model not in holdout comparison results: {model_name}")
    subset_threshold = [row for row in threshold_rows if str(row["Model"]) == model_name]
    return pick_best_model(subset, subset_threshold)


def select_winner_with_cv_stability(
    cv_result: object,
    results_df: pd.DataFrame,
    threshold_rows: list[dict[str, float | str]],
) -> tuple[pd.Series, dict[str, str | bool | int]]:
    """
    Pick a holdout winner that also ranks well on athlete CV.

    When CV leader and pure holdout winner disagree, prefer the candidate with the
    best combined CV + holdout rank among models that pass the recall hard gate on holdout.
    """
    operating_table = build_operating_points_table(results_df, threshold_rows)
    holdout_pure_row = pick_best_model(results_df, threshold_rows)
    holdout_pure = str(holdout_pure_row["Model"])
    cv_top = str(cv_result.summary.iloc[0]["Model"])

    cv_rank = {str(model): index + 1 for index, model in enumerate(cv_result.summary["Model"])}

    eligible: list[tuple[int, int, int, int, float, str]] = []
    for _, op in operating_table.iterrows():
        model_name = str(op["Model"])
        if not bool(op["pass_recall_hard"]):
            continue
        combined_rank = cv_rank.get(model_name, len(cv_rank) + 10) + int(op["Rank"])
        eligible.append(
            (
                combined_rank,
                cv_rank.get(model_name, len(cv_rank) + 10),
                int(op["Rank"]),
                int(op["Tier"]),
                -float(op["F1"]),
                model_name,
    ...
    if eligible:
        eligible.sort()
        selected_name = eligible[0][5]
        selection_reason = "cv_holdout_combined_rank"
    else:
        selected_name = holdout_pure
        selection_reason = "holdout_fallback_no_recall_gate"

    if selected_name == cv_top == holdout_pure:
        selection_reason = "cv_and_holdout_agree"

    selected_row = pick_best_model_for_name(results_df, threshold_rows, selected_name)
    agreement_info: dict[str, str | bool | int] = {
        "cv_top_model": cv_top,
        "holdout_pure_winner": holdout_pure,
        "holdout_winner": selected_name,
        "selected_model": selected_name,
        "agreement": selected_name == cv_top == holdout_pure,
        "selection_reason": selection_reason,
    }
    return selected_row, agreement_info
