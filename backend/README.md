...
- Daily athlete data and prediction outputs are read/written via **Firestore** (see `services/history/inference_bundle.py` + `persist.py`; `repository.py` is a thin re-export facade). There is **no** PostgreSQL layer in this backend.
- After `POST /predict/daily`, merged fields on `daily_health/{date}` include **`finalRiskScore`**, **`riskLevel`**, **`predictionConfidence`**, **`predictionUpdatedAt`** (see `services/history/persist.py`).
