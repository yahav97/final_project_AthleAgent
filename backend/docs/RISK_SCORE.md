...
6. [35 הפיצ'רים — טבלה מלאה](#6-35-הפיצרים--טבלה-מלאה)
...
| **כמה פיצ'רים?** | **35** — רשימה קבועה ב־`data/model_feature_contract.json` |
...
|-----|-----------|----------|
| **שינה** (`sleepMinutes`) | `daily_health/{D}` | אין |
| **עומס פיזי** (צעדים, מרחק, דופק, HRV…) | `daily_health/{D-1}` | אין — אין קריאה מ-`{D}` |

**סקר (`daily_checkins/{D}`):** אין fallback לאתמול — אם לא מילאו סקר, נכנסים defaults ניטרליים.

**תזונה:** `daily_nutrition/{D-1}`; שדות חסרים → ממוצעים מ-`nutrition_defaults.py`. `nutritionImputed=True` → קנס **−0.12** ב-`quality_score`.
...
sequenceDiagram
    ...
    API->>API: Preprocessing → 35 features
...
| `steps`, `distanceMeters`, `activeCalories`, `heartRate*`, `hrvRmssd`, … | `daily_health/{D-1}` בלבד |
...
### 5.4 תזונה (`daily_nutrition/{D-1}` + ממוצעים)
...
| 2 | `age` | גיל (נגזר מ-`birth_date`) | פרופיל |
...
| 28 | `acwr_ratio` | יחס עומס חד/בסיס (ACWR) | היסטוריה / proxy |
| 29 | `acwr_ratio_ma7` | ממוצע ACWR (proxy: = יום D) | proxy מיום נוכחי |
| 30 | `calorie_balance` | מאזן קלורי (צריכה−שריפה) | נגזר |
| 31 | `sleep_hours_ma7` | ממוצע שינה (proxy: = יום D) | proxy מיום נוכחי |
| 32 | `sleep_debt_3d` | חוב שינה 3 ימים | היסטוריה / proxy |
| 33 | `hrv_drop` | ירידת HRV מול ממוצע שבועי | היסטוריה / proxy |
| 34 | `load_recovery_imbalance` | חוסר איזון עומס/התאוששות | `acwr × sleep_debt` |
| 35 | `speed_intensity_ratio` | יחס עוצמת מהירות | `max_speed / avg_speed` |
...
acwr_baseline      = max(0.55, weekly_mean×0.85 + weekly_std×0.35 + 0.5)  # פנימי — לא פיצ'ר
acwr_ratio         = clamp(acute/acwr_baseline, 0.35, 2.8)
...
### 9.1 כל 35 הערכים (`DEFAULT_FEATURE_VALUES`)
...
| `workout_intensity_minutes` | 45 | `acwr_ratio_ma7` | 1.0 |
| `avg_cadence` | 168 | `acwr_ratio` | 1.0 |
| `elevation_gained_m` | 50 | `calorie_balance` | 0 |
| `floors_climbed` | 5 | `sleep_hours_ma7` | 7 |
| `avg_speed` | 8 | `sleep_debt_3d` | 1 |
| `max_speed` | 11 | `hrv_drop` | 0 |
| `avg_power` | 0 | `load_recovery_imbalance` | 1 |
| `active_calories_burned` | 350 | `speed_intensity_ratio` | 1.3 |
| `sleep_hours` | 7 | `hrv_score` | 62 |
| `resting_hr` | 54 | | |
| `respiratory_rate` | 15 | | |
| `spo2` | 97 | | |
...
|------|--------|--------|-------------|
| 1 | `hrv_drop` | 16.0% | ירידה ב-HRV לעומת הממוצע השבועי |
| 2 | `history_injury_count` | 5.3% | היסטוריית פציעות |
| 3 | `load_recovery_imbalance` | 3.8% | עומס גבוה × חוב שינה |
| 4 | `sleep_debt_3d` | 3.1% | חוב שינה מצטבר |
| 5 | `acwr_ratio` | 3.0% | יחס עומס חד/כרוני |
| 6 | `daily_distance_km` | 2.9% | מרחק יומי |
| 7 | `acwr_ratio_ma7` | 2.9% | ממוצע נע 7 ימים ל-ACWR |
| 8 | `stress_level` | 2.9% | סטרס מהסקר |
| 9–35 | (שאר הפיצ'רים) | ~1–3% כל אחד | תרומה קטנה יחסית |
...
**מה *לא* עובר לפרונט:** אף אחד מ-35 הפיצ'רי המודל (`acwr_ratio`, `stress_level`…), `data_quality_score`, או פירוט החסרים — אלה נשארים בשרת (לוגים בלבד).
...
חסמים קשים (מורידים ל-**מקסימום 0.25** — לא חוסמים HTTP):
- חסר **אות עומס:** אין `steps` **וגם** אין `distanceMeters` **וגם** אין `activeCalories` (ערך **> 0**)
...
| ותיק, תזונה ממוצעת (`nutritionImputed`) | 0.95 | 0.88 | (0.6×0.95+0.4×0.88)×100 | **92.2%** |
...
| סף | ערך | שימוש |
|----|-----|-------|
| Operating threshold | **from manifest** (currently **0.08**) | Recall@Threshold, Precision, FPR — **לא** לסיווג `risk_level` ב-production |
...
```
  ...
  risk_level + Android UI     │  Low ≤20 | Med 21–70 | High >70
                              │
    סף אימון (manifest)       │  threshold from run_manifest.json (Recall metrics only)
```
...
### למה עומס פיזי מאתמול?

בבוקר, נתוני עומס של **היום** עדיין חלקיים. החיזוי משתמש ב-`daily_health/{D-1}` — יום לוח שנה מלא אתמול. אין קריאה מ-`{D}` לעומס.

### ספורטאי חדש — מה קורה?

- פחות מ-4 ימי היסטוריה → defaults ניטרליים ל-ACWR/HRV
- אין תזונה אתמול → ממוצעים מ-`nutrition_defaults.py` + `nutritionImputed` (−0.12 confidence)
- `prediction_confidence` נמוך יותר
- החיזוי **עדיין רץ** — אבל פחות אמין

### האם תזונה משפיעה על הסיכון?

כן — דרך `nutrition_intake_calories`, `daily_calories`, `calorie_balance`. חשיבות יחסית נמוכה (~1.4%) לעומת HRV/סטרס/ACWR. תזונה ממוצעת (`nutritionImputed`) מורידה גם את `prediction_confidence` ב-**~4.8** נקודות (בהיסטוריה high).
...
| [`../../README.md`](../../README.md) | Run, config, API, tests |
